  ...
  <section class="page-stack">
    <header class="page-header">
      ...
      <div class="flex flex-wrap gap-2">
        <UButton v-if="auth.session.value.user?.type !== 'SUPER_ADMIN'" to="/register/company" :label="t('common.createCompany')" color="neutral" variant="outline" icon="i-lucide-building-2" />
    ...
    <section v-if="auth.session.value.user?.type !== 'SUPER_ADMIN'" class="operation-panel">
      ...
      <div v-if="auth.session.value.memberships.length" class="divide-y divide-[var(--drixal-line)]">
        <div v-for="membership in auth.session.value.memberships" :key="membership.id" class="flex flex-col items-start justify-between gap-3 p-4 sm:flex-row sm:items-center">
          <div class="min-w-0">
            ...
            <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-[var(--drixal-muted)]">
              ...
              <UBadge :label="t(`statuses.${membership.company.status}`)" :color="statusColor(membership.company.status)" variant="soft" size="sm" />
            </div>
          </div>
        </div>
      </div>
